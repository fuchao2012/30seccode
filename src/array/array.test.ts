...
describe('array 1', () => {
    ...
    it('最大的N个值，如果N大于1，则返回逆序排列的N个值组成的数组1', function () {
        // maxN([1, 2, 3]); // [3]
        const array = [1, 2, 3];
        const result = [3];
        expect(Array.MaxN(array)).to.eql(result);
    });
    it('最大的N个值，如果N大于1，则返回逆序排列的N个值组成的数组2', function () {
        // maxN([1, 2, 3], 2); // [3,2]
        const array = [1, 2, 3];
        const n = 2;
        const result = [3, 2];
        expect(Array.MaxN(array, n)).to.eql(result);
    });

    it('最小的N个值，如果N大于1，则返回正序排列的N个值组成的数组1', function () {
        // maxN([1, 2, 3]); // [3]
        const array = [1, 2, 3];
        const result = [1];
        expect(Array.MinN(array)).to.eql(result);
    });
    it('最小的N个值，如果N大于1，则返回正序排列的N个值组成的数组2', function () {
        // maxN([1, 2, 3], 2); // [3,2]
        const array = [1, 2, 3];
        const n = 2;
        const result = [1, 2];
        expect(Array.MinN(array, n)).to.eql(result);
    });

    it('如果过滤器函数对于数组中所有数据都返回false，则本函数返回true，否则返回false1', function () {
        // none([0, 1, 3, 0], x => x == 2); // true
        const array = [0, 1, 3, 0];
        const filter = x => x === 2;
        expect(Array.None(array, filter)).true;
    });

    it('如果过滤器函数对于数组中所有数据都返回false，则本函数返回true，否则返回false2', function () {
        // none([0, 1, 3, 0], x => x == 2); // true
        const array = [0, 0, 0];
        expect(Array.None(array)).true;
    });

    it('返回第N个值，如果N取负数则从尾部开始计算', function () {
        const array = ['a', 'b', 'b'];
        const n = -3;
        expect(Array.NthElement(array, n)).to.equal('a');
    });
});
